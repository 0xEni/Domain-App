...
contract Domains is ERC721URIStorage {
    ...
    // A Mapping to store string values
    mapping(string => string) public records;

    mapping (uint => string) public names;


    constructor(string memory _tld) payable ERC721("Enigma Name Service", "E-NS") {
  ...
  function price(string calldata name) public pure returns(uint) {
      ...
      require(len > 0 );
      if (len == 3) {
          return 5 * 10**17; // 5 Matic to 17 Decimals equals 0.5 Matic
      } else if (len == 4) {
  ...
  function register(string calldata name) public payable {
      // Require name to be unregistered and valid
      ...
      //Combine the name passed in the function with the tld
       ...
       string memory json = Base64.encode(
        bytes(
            string(
                ...
                '{"name": "',
                _name,
                '", "description": "A domain on the Enigma name service", "image": "data:image/svg+xml;base64,',
  ...
  // Returns Domain Owner's address
